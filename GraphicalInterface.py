...
def selected_files():
    ...
    for x in range(10):
        if (os.path.exists("./suspects"+"/"+list_images[x])) :
            files.append("./suspects"+"/"+list_images[x])
    return files
...
button_next = tk.Button(ImageFrame5,text=">>NEXT",fg="black",bg="white",height=2,width=3,command=lambda:ask_confirmation())#,command=lambda:remove_images()) il faudrait faire une commande qui envoie fichier dans algo pour en sortir de nouveaux
...
print()
ask_the_client= input("Did you select all the wanted files? [y/n]")
#print(ask_the_client)
if (ask_the_client=="y"):
    f=selected_files()
    print("Les fichiers sont",f)
